...
TARGET_DIR = Path(get_package_share_directory("kyon_locomotion")) / "checkpoints"
...
class LoadPolicyNode(Node):
    ...
    def load_policy(self):
        ...
        policy_dir = TARGET_DIR / self.__policy_name
        if not policy_dir.is_dir():
            ...
            subprocess.run(
                ["wget", url, "-O", archive], cwd=TARGET_DIR, check=True, stderr=sys.stderr, stdout=sys.stderr
            )
            subprocess.run(
